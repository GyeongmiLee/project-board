...
//    private ArticleRepository articleRepository;
    ...
    * */
    ...
    void selectTest() {
        ...
        List<Article> articles  =  articleRepository.findAll(); //모든 컬럼 조회 위해
        //여러개 받을꺼니까 list , 받을게 article이니까!
     ...
     *  엔티티 가서 클래스 레벨로 @EntityListeners(AuditingEntityListener.class) 걸어둠*/
    

    @DisplayName("insert 테스트")
    @Test
    void insertTest(){
        //기존꺼 article개수 세고, insert 후 , 기존거보다 현재꺼가 1 차이가 나면 insert 제대로 됐다는 뜻.
        long originalArticle = articleRepository.count();
        ...
        //현재 article개수 세기
        long currentArticle = articleRepository.count();

        assertThat(currentArticle).isEqualTo(originalArticle+1);
    ...
    void updateTest(){
       ...
       article.setTitle("이거 타이틀"); //바꿈
//        articleRepository.save(article); //save해놔야함!
       ...
       assertThat(article2).hasFieldOrPropertyWithValue("title", "이거 타이틀");
    }
    ...
    void deleteTest(){
        ...
        * - 순서
            ...
            .orElseThrow() -> 없으면 throw 시켜서 일단 테스트가 끝나게 하자
        * */
        long ori = articleRepository.count();
        long oriComment = articleCommentRepository.count(); // 댓글도 구해둠
        Article article = articleRepository.findById(2L).orElseThrow();
        int deletedCommentSize = article.getArticleComments().size();
        // 해당 게시글이 가지고 있는 댓글의 수
        // 나중에 "모든 댓글의 수 - 게시글에 딸려있는 댓글의 수" 하면 몇개 지워졌는지 알 수 있다.

        /* 3) delete 하고 (전체 게시글 개수 -1 됨) */
        articleRepository.delete(article);
        long curr = articleRepository.count(); // 딜리트 이후의 총 게시글 수

        assertThat(curr).isEqualTo(ori-1);
        assertThat(articleCommentRepository.count()).isEqualTo(oriComment - deletedCommentSize);
